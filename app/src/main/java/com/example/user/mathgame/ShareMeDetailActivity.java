...
public class ShareMeDetailActivity extends Activity {
        ...
        public boolean handleMessage(@NonNull Message message) {
            if (message.what == 101) {
                ...
                if (mDeleteShareBean.getCode() == 200) {
                    // 表示成功
                    finish();
                    Toast.makeText(ShareMeDetailActivity.this,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //表示传递过来的是melist
        meList.addAll((ArrayList<ShareReceiveBean.DataBean.GivesBean.DevicesBeanX>) getIntent().getSerializableExtra("list"));
        ...
        Log.e("TAG", "ShareMeDetailActivity: " + "\t" + meList + "\t" + name + "\t" + userId + "\t" + givesId);
        ...
        mAdapter.setOnViewClickListener(new BaseRecyclerViewAdapter.OnViewClickListener() {
            ...
            public void onViewClick(View view, int position) {
                 ...
                 *  // 选中状态并刷新  先去把布局改一下
                 */
                if (meList.get(position).isIscheck()) {
                    meList.get(position).setIscheck(false);
                } else {
                    meList.get(position).setIscheck(true);
                }
                mAdapter.resetDevices(meList);
            }
        });
        ...
        btn_remove_share_detail.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                for (int i = 0; i < meList.size(); i++) {
                    if (meList.get(i).isIscheck()) {
                        dataList.add(meList.get(i).getEquipmentId());
                    }
                }
                //数据已经拿到  接下来做网络请求 移除
